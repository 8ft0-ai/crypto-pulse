...
This is a retrospective record. It preserves the original issue, pull-request, workflow, and evidence-comment history while making the completed phase easier to understand.
...
Prove that a merged `valid-ok` source snapshot can produce a deterministic Markdown report PR, pass generated-report validation, and render through the static site generator without committing generated `_site/` output.
...
- A real merged `valid-ok` source snapshot can drive deterministic report generation.
...
- The deterministic report format is more readable while remaining source-grounded, testable, and non-LLM.
...
That manual approval did not invalidate the proof, because the generated PR was not merged until the validation job passed. However, it does create friction for a more autonomous scheduled loop.

## Carry-forward direction

Phase 3 should make generated report PRs self-proving before they are opened. The generating workflow should run the critical proof steps itself:
...
Downstream PR validation should then become defence in depth, not the only source of merge confidence.
