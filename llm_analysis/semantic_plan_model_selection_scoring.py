...
def _signature(claim: Mapping[str, Any]) -> str:
    ...
    return "|".join((str(claim.get("intent") or ""), ",".join(identifiers), str(claim.get("comparison_relation") or "")))
...
def _redundant_claim_count(claims: list[Mapping[str, Any]]) -> int:
    ...
    for claim in claims:
        ...
        if claim.get("intent") == "absolute_observation" and isinstance(identifiers, list) and len(identifiers) == 1:
            repeated_absolutes += any(str(identifiers[0]) in operands for operands in comparison_operands)
    return duplicates + repeated_absolutes


def evaluate_expectation(plan: Mapping[str, Any], expectation: CaseExpectation) -> ExpectationResult:
    claims = _claims(plan)
    selected = {
        str(identifier)
        for claim in claims
    ...
    required_hits = sum(identifier in selected for identifier in expectation.required_evidence_ids)
    if required_hits != len(expectation.required_evidence_ids):
        diagnostics.append("missing_required_evidence:" + ",".join(sorted(set(expectation.required_evidence_ids) - selected)))
    ...
    quality_hits = sorted({
        str(identifier)
        for claim in claims
        if claim.get("intent") == "data_quality_limitation" and isinstance(claim.get("evidence_ids"), list)
    ...
    if expectation.required_source_disagreement_ids:
        ...
        disagreement_ok = any(
            ...
            and claim.get("comparison_relation") in {"not_equal", "less_than", "greater_than"}
            and isinstance(claim.get("evidence_ids"), list)
    ...
    hard_pass = required_hits == len(expectation.required_evidence_ids) and not forbidden_hits and not quality_hits and disagreement_ok
    denominator = len(expectation.required_evidence_ids) + int(bool(expectation.required_source_disagreement_ids))
    numerator = required_hits + int(disagreement_ok and bool(expectation.required_source_disagreement_ids))
    ...
    materiality = max(0.0, 1.0 - len(discouraged_hits) / max(1, len(expectation.discouraged_evidence_ids)))
    redundant = _redundant_claim_count(claims)
    count_restraint = min(1.0, expectation.maximum_claims / max(expectation.maximum_claims, len(claims)))
...
def stability(rows: Iterable[Mapping[str, Any]]) -> float:
    ...
    for row in rows:
        grouped.setdefault(str(row["case_key"]), []).append(set(row.get("claim_signatures", [])))
    scores = [
